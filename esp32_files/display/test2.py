...
fb.text('Temperature: 25 C', 10, 30,black)
...
buf_black2 = bytearray(w * h // 8)
fb3 = framebuf.FrameBuffer(buf_black2, h, w, framebuf.MONO_VLSB)
fb3.fill(white)
fb3.text('Simon\'s Weather Station!', 10, 90,black)

buf_epaper_black2 = bytearray(w * h // 8)

x=0; y=0; n=1; R=0
for i in range(1, 14):
    for j in range(1, 213):
        R = (n-x)+((n-y)*12)
        buf_epaper_black2[R-1] = buf_black2[n-1]
        n +=1
    x = n+i-1
    y = n-1

fb.blit(fb3, 10, 90)
e.display_frame(buf_epaper_black2, buf_epaper_red)
